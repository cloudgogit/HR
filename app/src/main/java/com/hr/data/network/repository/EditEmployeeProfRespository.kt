...
private class EditEmployeeProfRespositoryImpl(private val api : EmployeeProfApi) :
    EditEmployeeProfRespository {
    ...
    override suspend fun editeproof(
        ...
        withContext(Dispatchers.IO) {
            try {
                val response = api.EditEmployeeStep( editProfRequest = editProfRequest)
                if (response.isSuccessful) {
                    response.body()?.let {
                        if (it.status.equals("success"))
                            withContext(Dispatchers.Main) { onSuccess(it) }
...
interface EditEmployeeProfRespository {
    ...
    companion object Factory {
        fun create(api: EmployeeProfApi): EditEmployeeProfRespository =
            EditEmployeeProfRespositoryImpl(api)
    }
