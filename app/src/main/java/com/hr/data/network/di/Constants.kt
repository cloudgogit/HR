package com.hr.data.network.di

object Constants {
    const val IMAGE_FROM_GALLERY = 1
    const val IMAGE_FROM_CAMERA = 2
    const val IMAGE_DIRECTORY = "userPic"
    const val REQUEST_IMAGE_CAPTURE = 1

}

object API {
    const val INDEX = "index"
    const val USER_EXISTS = "user_exists"
    const val UPDATE_PROFILE = "update_profile"
    const val Home_API = "homescreen_api"
    const val Category_list_Api = "category_list"
    const val Add_new_store = "add_store_community"
    const val Search_User_Api = "users_search_list"
    const val Search_Store_Api = "stores_search_list"
    const val Favourites_List = "favourites_list"
    const val GET_NEARBY_STORE_LIST="get_nearby_stores_list"
    const val CLOSEST_TO_API="closest_to_you_search_list"
    const val PROFILE_SETTING="profile_settings_details"
    const val MY_SPACEE="my_space"
    const val VIEW_STORE_DETAILS="view_store_detail"
    const val ADD_FAV="add_favourite"
    const val VIEW_SPACE_DETAIL="view_space_detail"
    const val CREATE_POST="create_post"

}

object Preference {
    const val TOKEN = "token"
    const val SESSION_ID = "session_id"
    const val USER_STATUS = "user_status"
    const val IS_LOGGED_IN = "is_logged_in"
}

object Extras {
    const val AUTHORIZATION = "Authorization"
    const val DEVICE_ID = "deviceid"
    const val HEADER_SESSION_ID = "SessionId"
    const val HEADER_LANGUAGE = "Language"
    const val FROM_LOGIN_TO_HOME = true
}

object BundleConstants {
    const val COUNTRY_CODE = "country_code"
    const val PHONE_NUMBER = "phone_number"
    const val IS_BUSINESS = "is_business"
}

object ImageConstants {
    const val GALLERY = 1
    const val CAMERA = 2
    const val FILE_ = 3
    const val IMAGE_DIRECTORY = "Docs"
    const val MULTIPLE_IMAGE = 26
}
